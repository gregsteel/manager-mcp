...
async def _existing_dedup_keys(
    ...
    The fuzzy amount is `lines[*].amount` summed, not `fixedTotalAmount`:
    `fixedTotalAmount`/`fixedTotal` are `/api4`-only fields with no `/api2`
    equivalent, so any `/api2` PUT (e.g. `update_payment` reconciling the row
    against a supplier) resets them to 0/false on the underlying record even
    though `/api2`'s own merge-before-PUT leaves `Lines`/`CustomFields2`
    intact. Trusting `fixedTotalAmount` made every update-touched bank-feed
    row fuzzy-invisible too -- confirmed live 2026-09-20 when a payment whose
    dedup field had already been wiped by an update also read back
    `fixedTotalAmount: 0`, so it duplicated instead of fuzzy-matching."""
    ids: set[str] = set()
    ...
    for entity in ("receipt-batch", "payment-batch"):
        ...
        while True:
            ...
            for entry in entries:
                ...
                amount = record.get("fixedTotalAmount") or sum(
                    float(line.get("amount") or 0) for line in record.get("lines") or []
                )
