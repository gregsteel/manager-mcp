...
| `MANAGER_MCP_BANK_FEED_CONFIG_PATH` | no | Where provider config (below) is saved/read. Default `/app/feeds.config` (this app's `WORKDIR`); a deployment can point it at a mounted volume instead. See [Config lives in a file, not env vars](#config-lives-in-a-file-not-env-vars). |
...
The default path, `/app/feeds.config`, is this app's own writable `WORKDIR` — fine for a standalone container. A deployment that wants the file to survive container recreation, or to be shared/inspected from the host, sets `MANAGER_MCP_BANK_FEED_CONFIG_PATH` to a mounted volume instead.
