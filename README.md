...
- **When changes take effect:** the file is read fresh on every attempt, so saving a change (new credentials, a corrected account link, switching provider) applies on the next scheduled sync or the next `sync_bank_feeds` tool call — no restart needed.
